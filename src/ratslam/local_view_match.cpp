...
LocalViewMatch::LocalViewMatch(double vt_min_patch_normalisation_std, int vt_patch_normalise, 
                               double vt_normalisation, int vt_shift_match, int vt_step_match,
                               int vt_panoramic, double vt_match_threshold, int template_x_size,
                               int template_y_size, int image_crop_x_min, int image_crop_x_max,
                               int image_crop_y_min, int image_crop_y_max)
{
  VT_MIN_PATCH_NORMALISATION_STD = vt_min_patch_normalisation_std;
  VT_PATCH_NORMALISATION = vt_patch_normalise;
...
void LocalViewMatch::on_image(const unsigned char *view_rgb, bool greyscale, unsigned int image_width, unsigned int image_height)
{
  ...
  if (vt_error <= VT_MATCH_THRESHOLD)
  {
    ...
    cout << "VTM[" << setw(4) << get_current_vt() << "] Total VTs: " << templates.size() << endl;
    cout.flush();
  }
  else
  {
    ...
    cout << "VTN[" << setw(4) << get_current_vt() << "] Total VTs: " << templates.size() << endl;
    cout.flush();
  }
